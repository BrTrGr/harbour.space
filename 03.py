...
class UniqueConsecutiveIterator:
    ...
    def __next__(self) -> Any:
        while True:
            ...
            if not self.has_prev or value != self.prev_value:
                self.has_prev = True
...
def log_calls(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def call(*args: Any, **kwargs: Any) -> Any:
        ...
        kwargs_str = [f"{i}={j}" for i, j in kwargs.items()]
        signature = ", ".join(args_str + kwargs_str)
        result = func(*args, **kwargs)
        print(f"{func.__name__}({signature}) -> {result}")
        return result
...
def ensure_non_negative(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    3
    """
    def only_pos(*args: Any, **kwargs: Any) -> Any:
        result = func(*args, **kwargs)
        if isinstance(result, (int, float)) and result < 0:
            raise ValueError
        return result
    only_pos.__name__ = func.__name__
    return only_pos
